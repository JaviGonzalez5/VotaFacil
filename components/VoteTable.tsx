...
function VoteCell({ value }: { value: VoteValue | undefined }) {
  ...
  return (
    <td className={cn('px-3 py-3 text-center', config.cell)}>
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200">
            <th className="sticky left-0 bg-gray-50 px-4 py-3 text-left font-semibold text-gray-700 min-w-[120px] z-10">
              Participante
            </th>
            {options.map((option) => (
              <th
                key={option.id}
                className={cn(
                  'px-3 py-3 text-center font-medium text-gray-700 min-w-[110px]',
              ...
              >
                <div className="flex flex-col items-center gap-1">
                  ...
                  {bestOptionId === option.id && (
                    <Badge className="text-[10px] px-1.5 py-0">⭐ Mejor</Badge>
                  )}
                </div>
              </th>
            ))}
        ...
        <tbody>
          {participants.map((participant, idx) => (
            ...
            >
              <td className="sticky left-0 bg-inherit px-4 py-3 font-medium text-gray-900 z-10">
                {participant.name}
              </td>
              {options.map((option) => (
                <VoteCell
        ...
        <tfoot>
          <tr className="bg-gray-50 border-t-2 border-gray-200">
            <td className="sticky left-0 bg-gray-50 px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide z-10">
              Resumen
            </td>
            {options.map((option) => {
              const c = countForOption(option.id)
              return (
                <td key={option.id} className={cn('px-3 py-3', bestOptionId === option.id && 'bg-indigo-50/60')}>
